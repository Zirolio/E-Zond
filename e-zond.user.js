...
class Settings {
    constructor() {
        ...
        window._client.necroCooldowns.on = window.storage.get('necroCooldowns');
    ...
    createSettings() {
        ...
        const onload = () => {
            ...
            shadow.getElementById('necroCooldowns').checked = window._client.necroCooldowns.on;
    ...
    show$hideSettings() {
        ...
        if (!settE.style.display) {
            ...
            window.storage.set('necroCooldowns', window._client.necroCooldowns.on);
...
class Friends {
    ...
    onlineController(name, server, add=true) {
        ...
        if (!add) plb && plb.remove();
            ...
            ct.id = `${name}-online`;
            ct.innerHTML = `<a href="https://evades.io/profile/${name}" class="hover:underline" target="_blank">${name}</a><span class="text-[hsl(172,69%,55%)] hover:text-[hsl(172,69%,36%)] hover:underline underline-offset-2 cursor-pointer" onclick="window._client.playProcessFn('${server.toLocaleLowerCase().slice(0, 2) == 'na' ? null : 'https://eu.evades.io'}', ${parseInt(server.toLocaleLowerCase().slice(2)) - 1}, 0)">${server}</span>`;
...
const sheildAIM = (md, maxDist=10) => {
    ...
    /*const velVec = { ...md }, toObj = { x: obj.x - me.x, y: obj.y - me.y };
    const dAngle = Math.atan2(velVec.y, velVec.x) - Math.atan2(toObj.y, toObj.x);


    const dot = velVec.x * toObj.x + velVec.y * toObj.y;
    const velVecL = Math.sqrt(velVec.x**2 + velVec.y**2), toObjL = Math.sqrt(toObj.x**2 + toObj.y**2);
    const angleBV = Math.acos(dot / (velVecL * toObjL));
    const velAnglNew = angleBV > Math.PI / 4 ? angleBV + (Math.PI / 2 - angleBV) : -angleBV;
    console.log(velAnglNew, { x: Math.floor(Math.cos(velAnglNew) * velVecL), y: Math.floor(Math.sin(velAnglNew) * velVecL) }, md)*/

    const   v = { x: -Math.floor(obj.x - me.x), y: -Math.floor(obj.y - me.y), updated: true }/* || { x: Math.floor(Math.cos(velAnglNew) * velVecL), y: Math.floor(Math.sin(velAnglNew) * velVecL), updated: true }*/, //,
            d = Math.sqrt((obj.x - me.x)**2 + (obj.y - me.y)**2);
...
const onMess = (msg) => {
    ...
    window._client.necroCooldowns.necroCooldown(msg);
...
const onDrawPlayer = (player) => {
    const center = { x: window._client.canvas.width / 2, y: window._client.canvas.height / 2 }, c = window._client.c;

    if (window._client.necroCooldowns.on && player.heroType == 4) {
        const id = Object.entries(window._client.user.globalEntities).filter(e => e[1].name == player.name)[0][0];
        if (id) {
            c.beginPath();
            c.fillStyle = 'rgb(253, 240, 193, 0.55)';
            c.arc(player.x - window._client.user.self.entity.x + center.x, player.y - window._client.user.self.entity.y + center.y, player.radius, -Math.PI / 2, window._client.necroCooldowns.storage[id] - Math.PI / 2, true);
            c.lineTo(player.x - window._client.user.self.entity.x + center.x, player.y - window._client.user.self.entity.y + center.y);
            c.closePath();
            c.fill();
        }
    }
...
const necroCooldown = (msg) => {
    const necrosData = msg.entities.filter(e => window._client.user.globalEntities[e.id]?.heroType == 4 && e.abilityOne?.cooldown);
    const totalNecroResCd = 0.07500000298023224;
    for (const necroData of necrosData) {
        window._client.necroCooldowns.storage[necroData.id] = Math.PI * 2 * (1 - necroData.abilityOne.cooldown / totalNecroResCd) || Math.PI * 2;// + Math.PI / 2;
    }
...
const client = {
    ...
    updateParam: (param, e) => {
        if (e.valueAsNumber) {
            ...
            if (param == 'necroCooldowns') window._client.necroCooldowns.on = e.checked;
    ...
    defaultStorage: () => {
        window.storage = {
            ...
            default: () => {
                ...
                if (localStorage.getItem('necroCooldowns') === null) window.storage.set('necroCooldowns', false);
...
console.log('Offed')
    if (e.target.type === 'module' && e.target.src.match(/\/index\.[0-9a-f]{8}\.js/)) console.log('Offed'), e.preventDefault();
});*/
// window.addEventListener('beforescriptexecute', e => console.log(e));
// Friends list
...
const _obsJs = new MutationObserver((ev) => {
    ...
    req.onload = () => {
        ...
        window.ccc = code;
        /*title: this.props.player.name + " [" + this.props.area + "]",*/
        code = code // 3sstr
            ...
            .replace(/\b(.)\.render\(this\.context,\s*this\.camera\)/g, (_, a) => _ + `, ${a}.isPlayer && window._client.onDrawPlayer(${a});`)
