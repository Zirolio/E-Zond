...
// @version      5
// @description Hi to all readers
// @description:ru Привет всем читающим
// @author       .zirolio.
...
// @grant        none
// @downloadURL https://raw.githubusercontent.com/Zirolio/E-Zond/blob/main/e-zond-beta.js
// @updateURL https://raw.githubusercontent.com/Zirolio/E-Zond/blob/main/e-zond-beta.js
// ==/UserScript==
...
class Settings {
    ...
    createSettings() {
        ...
        xhr.open('GET', 'https://cdn.jsdelivr.net/gh/Zirolio/E-Zond/settings/index.html', true)
        xhr.send();
    }
...
const AuRes = (dt) => {
    if (window._client.user.heroInfoCard.heroType !== 9 || (dt || window._client.user.self.entity.deathTimer) / 1000 <= 56.5) window._client.chrono.aur.useOn = null;
        ...
        if (!window._client.chrono._chronoShadows.filter(s => s.id == window._client.chrono.aur.useOn.id - window.mis)[0] && chekCanUse(window._client.user.heroInfoCard.abilityOne, true)) {
...
// -----
// Reaper
const reaperShadow = (msg) => {
    console.log('->', msg);
    /*const entity = window._client.user.self.entity;
    if (!entity || entity.heroType !== 10) return;
    window._client.user.latestServerSelfEntity.isDeparted*/
    // if (msg.globalEntities[0] && msg.globalEntities[0].effects) console.log(msg)
    // window._client.reaper.showReaperShadow
}
// ------
// Auto use Abilitys
...
window.onwheel = (e) => {
    if (e.toElement == document.getElementById('canvas')) {
        ...
        document.getElementById("chb_14").value = window._client.zoom._new * 100;
        reZoom();
    }
}
// ---------
...
const onNewDataFromServer = (e) => {
    ...
    Object.values(e.entities).forEach(v => {
        /* const tI1 = window._client.setIgnis.player === v.id; // DELETED!!!
        if (tI1 && v.abilityTwo) { window._client.setIgnis.ignisKD = v.abilityTwo.cooldown;} */
...
const greenBalls = (msg) => {
    ...
    if (Object.keys(window._client.greenBalls.ballsFrames).length == 0) {
        greenBalls.forEach(ballD => {
            window._client.greenBalls.ballsFrames[ballD[0]] = { ids: [], i: 0 }
            for (let i = 0; i < window._client.greenBalls.maxGreenBalls; i++) {
                window._client.greenBalls.ballsFrames[ballD[0]].ids.push(`${-ballD[0]}-${i}`);
                msg.entities.push({
                    x: ballD[1].x,
                    y: ballD[1].y,
                    id: `${-ballD[0]}-${i}`,
        ...
        greenBalls.forEach(ballD => {
            msg.entities.push({
                x: ballD[1].x,
                y: ballD[1].y,
                id: `${-ballD[0]}-${window._client.greenBalls.ballsFrames[ballD[0]].i}`,
                brightness: 0.24,
                radius: ballD[1].radius,
                // entityType: 10,
            });
            window._client.greenBalls.ballsFrames[ballD[0]].i = window._client.greenBalls.ballsFrames[ballD[0]].i + 1 == window._client.greenBalls.maxGreenBalls ? 0 : window._client.greenBalls.ballsFrames[ballD[0]].i + 1;
        });
    }
    /*greenBalls.forEach(ballD => {

        const ball = ballD[1];
        if (!window._client.greenBalls.ballsFrames[-ballD[0]]) window._client.greenBalls.ballsFrames[-ballD[0]] = 1;
        if (window._client.greenBalls.ballsFrames[-ballD[0]] == window._client.greenBalls.maxGreenBalls + 1) return;
        msg.entities.push({
            x: ball.x,
            y: ball.y,
            id: `${-ballD[0]}-${window._client.greenBalls.ballsFrames[-ballD[0]]}`,
            brightness: 0.24,
            radius: ball.radius,
            entityType: 10,
        });
        window._client.greenBalls.ballsFrames[-ballD[0]]++;
    });*/
}
...
const yellowBallsEEH = (msg) => {
    ...
    yellowBalls.forEach(ballD => {
        ...
        /*
        if (window._client.yellowBallsEEH.yellowBallsCords[ballD[0]] == undefined) window._client.yellowBallsEEH.yellowBallsCords[ballD[0]] = []; // V2
        if (!window._client.yellowBallsEEH.yellowBallsCords[ballD[0]].length) {
            window._client.yellowBallsEEH.yellowBallsCords[ballD[0]].push({ x: ballD[1].x, y: ballD[1].y });
            msg.entities.push({
                x: window._client.yellowBallsEEH.yellowBallsCords[ballD[0]][0].x,
                y: window._client.yellowBallsEEH.yellowBallsCords[ballD[0]][0].y,
                id: -ballD[0],
        ...
        } else if (window._client.yellowBallsEEH.yellowBallsCords[ballD[0]].length == 1) {
            if (window._client.yellowBallsEEH.yellowBallsCords[ballD[0]][0].x !== ballD[1].x || window._client.yellowBallsEEH.yellowBallsCords[ballD[0]][0].y !== ballD[1].y) window._client.yellowBallsEEH.yellowBallsCords[ballD[0]].push({ x: ballD[1].x, y: ballD[1].y });
        } else {
            if (window._client.yellowBallsEEH.yellowBallsCords[ballD[0]][1].x !== ballD[1].x || window._client.yellowBallsEEH.yellowBallsCords[ballD[0]][1].y !== ballD[1].y) {
                msg.entities.push({
                    x: window._client.yellowBallsEEH.yellowBallsCords[ballD[0]][1].x,
                    y: window._client.yellowBallsEEH.yellowBallsCords[ballD[0]][1].y,
                    id: -ballD[0],
                    brightness: 0.4,
                    radius: ballD[1].radius,
                    entityType: 55,
                });
                window._client.yellowBallsEEH.yellowBallsCords[ballD[0]][1] = { x: ballD[1].x, y:  ballD[1].y };
            }
        }*/
    });
}
...
const onMess = (msg) => {
    ...
    // if (window._client.chrono.aur.on) window._client.chrono.aur.AuRes(me ? me.deathTimer : null);
};
// ------------------------
...
const _obs = new MutationObserver((ev) => {
    ...
    elem.remove();
    code = code
        // .replace(/(if\s*\((.)\.isDeparted)\)/g, (_, a, b) => { return _ + `window._client.reaper.reaperShadow(${b});` + a + '&& 0)' })
    ...
    // .replace('this.sendInputs()', 'console.log(this.self.entity === void 0),this.sendInputs()')

        // .replace(/(if\s*\(a\.isDeparted\))/g, a => { return 'if (a.isDeparted && a.heroType !== 10)' })

        // .replace(/(this\.renderer\.clear\(\),)/g, a => { return 'window._client.gameState = this.gameState,' + a })
        // .replace(/(this\.randomFlickerRadius\s*=\s*)(\d+)(,\s*this\.flickerChance\s*=\s*)(\.\d+)/g, (_, a, na, b, nb) => { compleeted++; return a + `window._client.flashinBalls ? ${na} : 0` + b + `window._client.flashinBalls ? ${nb} : 0` })
        // '#939393') return
        // .replace(/(this\.flipped\s*\?)/g, a => { compleeted++; return 'return;' + a })
    // mist_enemy
    // phantom_enemy
    // firefly_enemy
    // |mist_enemy|phantom_enemy|firefly_enemy
        // |------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
// \btiles:
